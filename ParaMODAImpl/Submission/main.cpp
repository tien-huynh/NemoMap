...
int main(int argc, char *argv[])
{
   if (argc == 5)
   {
	  ...
	  if (string(argv[1]) == "-g" && string(argv[3]) == "-h")
	  {
		 ...
		 cout << "\nInput Graph (G): Nodes - " << boost::num_vertices(inputGraph.graph) << "; Edges: " << boost::num_edges(inputGraph.graph) << endl;

		 //reads query file 
		 QueryGraph queryGraph = GraphProcessor().LoadGraph(queryfile, true);
		 cout << "\nQuery Graph (H): Nodes - " << boost::num_vertices(queryGraph.graph) << "; Edges: " << boost::num_edges(queryGraph.graph) << "\n" << endl;
